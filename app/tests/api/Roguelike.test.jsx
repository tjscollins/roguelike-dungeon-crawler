...
describe('Roguelike API', () => {
  ...
  it('should create the starting room of the level', () => {
    var level = Roguelike.randomLevel(15, 15, true).map.map((val) => {
    ...
    level = Roguelike.randomLevel(25, 25, true).map.map((val) => {
      return val.filter((valN) => {
        return valN !== 0;
      });
    }).filter((val) => {
    ...
    expect(level[0][0]).toEqual(1);
    level = Roguelike.randomLevel(30, 10, true).map.map((val) => {
      return val.filter((valN) => {
        return valN !== 0;
      });
    }).filter((val) => {
